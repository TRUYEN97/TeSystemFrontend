...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  const [internalSelected, setInternalSelected] =
    useState<ValueType>(defaultSelected);
  ...
  const updateSelection = (newSelected: ValueType) => {
  ...
  const handleSelect = (optionValue: string | number) => {
    const newSelected = selectedOptions.includes(optionValue as never)
      ? selectedOptions.filter((v) => v !== optionValue)
      : ([...selectedOptions, optionValue] as ValueType);

    updateSelection(newSelected as ValueType);
  };

  const removeOption = (optionValue: string | number) => {
    updateSelection(
      selectedOptions.filter((v) => v !== optionValue) as ValueType,
    );
  };
  ...
  return (
    <div className="w-full" ref={dropdownRef}>
      ...
      <div className="relative z-99 inline-block w-full">
